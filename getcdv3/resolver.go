...
func GetConfigConn(serviceName string, operationID string) *grpc.ClientConn {
	...
	//1
	if config.Config.RpcRegisterName.SCUserName == serviceName {
		configPortList = config.Config.RpcPort.SCUserPort
	}
...
func GetDefaultGatewayConn4Unique(schema, etcdaddr, operationID string) []*grpc.ClientConn {
	...
	if IsUpdateStart == false {
		Conn4UniqueList = getConn4Unique(schema, etcdaddr, config.Config.RpcRegisterName.RelayName)
		go func() {
			for {
				select {
	...
	log.NewWarn(operationID, utils.GetSelfFuncName(), " len(grpcConns) == 0 ", schema, etcdaddr, config.Config.RpcRegisterName.RelayName)
	grpcConns = GetDefaultGatewayConn4UniqueFromcfg(operationID)
	log.NewDebug(operationID, utils.GetSelfFuncName(), config.Config.RpcRegisterName.RelayName, grpcConns)
	return grpcConns
}
...
func GetDefaultGatewayConn4UniqueFromcfg(operationID string) []*grpc.ClientConn {
	...
	configPortList := config.Config.RpcPort.GatewayPort
